//! Проверяет model-visible payload обычного Responses и Responses Lite.
...
/// Проверяет порядок effective developer instructions в одной секции developer message.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
/// Проверяет порядок файловых инструкций модели в top-level поле обычного Responses-запроса.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
/// Проверяет раздельные developer messages для base и developer instructions в Responses Lite.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_lite_separates_model_instructions_files_and_developer_instructions_files() {
    skip_if_no_network!();
    let server = MockServer::start().await;
...
"#,
            ...
            std::fs::write(home.join("model-b.md"), "Second model file.\n\n")
                .expect("write second model instructions file");
            std::fs::write(home.join("developer-a.md"), "\nFirst developer file.\n")
                .expect("write first developer instructions file");
            std::fs::write(home.join("developer-b.md"), "Second developer file.\n\n")
                .expect("write second developer instructions file");
        });
    let codex = builder
        .build(&server)
    ...
    let expected_model_instructions = "First model file.\n\nSecond model file.";
    let expected_developer_instructions =
        "Inline developer instructions.\n\nFirst developer file.\n\nSecond developer file.";
    let instruction_groups = request
        .message_input_text_groups("developer")
        .into_iter()
        .filter_map(|group| {
            let instruction_texts = group
                .into_iter()
                .filter(|text| {
                    text == expected_model_instructions || text == expected_developer_instructions
                })
                .collect::<Vec<_>>();
            (!instruction_texts.is_empty()).then_some(instruction_texts)
        })
        .collect::<Vec<_>>();
    assert_eq!(
        instruction_groups,
        vec![
            vec![expected_model_instructions.to_string()],
