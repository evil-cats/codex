//! Собирает replacement history и канонический initial context для local compaction.
...
async fn run_compact_task_inner_impl(
    ...
    let user_messages = collect_annotated_user_messages_excluding(history_items, |item| {
...
pub(crate) fn collect_annotated_user_messages(
    ...
    collect_annotated_user_messages_excluding(items, |_| false)
}

/// Собирает user messages с metadata без extension-owned prompt state.
pub(crate) fn collect_annotated_user_messages_excluding(
    items: &[ResponseItemEnvelope],
    mut should_exclude: impl FnMut(&ResponseItem) -> bool,
) -> Vec<CompactedUserMessage> {
    items
        .iter()
        .filter(|envelope| !should_exclude(&envelope.item))
        .filter_map(|envelope| compacted_user_message(&envelope.item, envelope.metadata.clone()))
        .collect()
}

/// Преобразует реальное user item в summary input, сохраняя его metadata sidecar.
