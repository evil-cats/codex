//! Проверяет сборку replacement history и размещение initial context при compaction.
...
/// Проверяет, что фильтрация prompt state не теряет metadata оставшихся user messages.
#[test]
fn collect_annotated_user_messages_excluding_preserves_retained_metadata() {
    let retained = ResponseItemEnvelope {
        item: user_message("retained"),
        metadata: Some(CodexHarnessMetadata::default()),
    };
    let excluded = ResponseItemEnvelope::new(user_message("extension context"));

    let collected = collect_annotated_user_messages_excluding(&[retained, excluded], |item| {
        matches!(
            item,
            ResponseItem::Message { content, .. }
                if content.iter().any(|content| {
                    matches!(content, ContentItem::InputText { text } if text == "extension context")
                })
        )
    });
