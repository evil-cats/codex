...
async fn developer_instructions_files_reject_missing_file() -> std::io::Result<()> {
    ...
    let expected_message_prefix = format!(
        "failed to read developer instructions file {}:",
        missing_path.display()
    );
    assert!(message.contains(&expected_message_prefix), "{message}");
...
#[tokio::test]
async fn load_config_resolves_tui_terminal_title_label() {
    let cfg = toml::from_str::<ConfigToml>(
        r#"
[tui]
...
"#,
    ...
    .expect("TUI config with terminal title label should deserialize");
    ...
    assert_eq!(config.tui_terminal_title_label.as_deref(), Some("hermione"));
}
