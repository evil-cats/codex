...
impl ToolRegistry {
    ...
    pub(crate) async fn dispatch_any_with_terminal_outcome(
        ...
        // Пользовательская activity начинается после того, как pre-tool hooks разрешили вызов,
        // но до запуска обработчика; завершается она по тому же результату, который идёт модели.
