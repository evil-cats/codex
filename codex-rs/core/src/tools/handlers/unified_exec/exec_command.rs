...
impl ExecCommandHandler {
    async fn handle_call(
        ...
        .await;
        // При `Ok(None)` резервирование сохраняется: обычному запуску ниже всё ещё нужен этот ID.
        ...
        match result {
            ...
            // Эти ошибки содержат только ограниченные метаданные; передаём их без общей
            // обёртки, чтобы отклонение `Promise` сохранило контракт Code Mode.
            Err(
...
/// Сужает интерактивную схему до одноразового запуска и явно фиксирует EOF после `stdin`.
fn one_shot_exec_command_spec(spec: ToolSpec) -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::string(Some(
            "Optional UTF-8 text to write to the process standard input immediately after launch. Use it for data that would otherwise require a separate shell producer or pipeline. No newline is added. Stdin is closed after the text is written so the process observes EOF."
                .to_string(),
        )),
    );
    properties.insert(
