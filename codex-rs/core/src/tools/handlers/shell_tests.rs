...
/// Проверяет, что command, cwd, shell policy и runtime identity берутся из
/// выбранного environment без отката к общему thread config.
#[tokio::test]
async fn shell_command_handler_to_exec_params_uses_selected_environment() {
    ...
    let mut expected_env = create_env_with_runtime(
