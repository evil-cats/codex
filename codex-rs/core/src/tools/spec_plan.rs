...
fn add_core_utility_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if turn_context.config.get_system_time_enabled {
        registry.add(SystemTimeHandler);
    }
    if turn_context.config.get_thread_info_enabled {
        registry.add(ThreadInfoHandler);
    }
