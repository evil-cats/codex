...
/// Config gate исключает статический tool и из registry, и из spec, видимой модели.
#[tokio::test]
async fn system_time_tool_respects_config_gate() {
    let enabled = probe(|_| {}).await;
    enabled.assert_visible_contains(&["get_system_time"]);
    enabled.assert_registered_contains(&["get_system_time"]);

    let disabled = probe(|turn| {
        update_config(turn, |config| {
            config.get_system_time_enabled = false;
        });
    })
    .await;
    disabled.assert_visible_lacks(&["get_system_time"]);
    disabled.assert_registered_lacks(&["get_system_time"]);
}

/// Инструмент fork остаётся доступным рядом с независимым upstream namespace `clock`.
#[tokio::test]
async fn system_time_tool_coexists_with_upstream_clock_namespace() {
    let plan = probe(|turn| {
    ...
    plan.assert_visible_contains(&["get_system_time", "clock"]);
    assert_eq!(
...
/// Настройка исключает `get_thread_info` и из registry, и из spec, видимой модели.
#[tokio::test]
async fn thread_info_tool_respects_config_gate() {
    let enabled = probe(|_| {}).await;
    enabled.assert_visible_contains(&["get_thread_info"]);
    enabled.assert_registered_contains(&["get_thread_info"]);

    let disabled = probe(|turn| {
        update_config(turn, |config| {
            config.get_thread_info_enabled = false;
        });
    })
    .await;
    disabled.assert_visible_lacks(&["get_thread_info"]);
    disabled.assert_registered_lacks(&["get_thread_info"]);
}
...
/// Проверяет прямую видимость `read_file` без публикации во вложенном `exec`.
#[tokio::test]
async fn read_file_stays_direct_and_outside_code_mode() {
    let plan = probe(|turn| {
        set_features(turn, &[Feature::CodeMode, Feature::CodeModeOnly]);
    })
    .await;
    ...
    assert_eq!(plan.exposure("read_file"), ToolExposure::DirectModelOnly);

    let ToolSpec::Freeform(exec) = plan.visible_spec(codex_code_mode::PUBLIC_TOOL_NAME) else {
        panic!("expected code mode exec tool");
    };
    assert!(!exec.description.contains("read_file"));
}
