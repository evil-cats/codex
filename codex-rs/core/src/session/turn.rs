...
pub(crate) async fn run_turn(
    ...
    let initial_world_state_delivery = initial_world_state_delivery?;
    ...
    loop {
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if should_roll_over {
                    // A tool call can change extension-owned state in the same sampling step that
                    // triggers compaction. Install that fresh state in the replacement history,
                    // while preserving any one-shot transition fragment for the next sampling.
                    let compacted_world_state = Arc::new(
                        sess.build_world_state_for_step(step_context.as_ref())
                            .await?,
                    );
                    let (_, next_sampling_fragments) = compacted_world_state
                        .render_diff_for_sampling(&world_state.snapshot())
                        .into_delivery();
                    next_sampling_world_state_items.extend(
                    ...
                    if let Err(err) = run_auto_compact(
                        ...
                        InitialContextInjection::BeforeLastUserMessage {
                            world_state: Arc::clone(&compacted_world_state),
                    ...
                    world_state = compacted_world_state;
                    if run_pending_session_start_hooks(&sess, &turn_context).await {
                        return Ok(None);
                    }
