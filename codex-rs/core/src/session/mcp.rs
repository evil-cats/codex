...
impl Session {
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        ...
        loop {
            ...
            // Dirty остаётся pending: turn продолжает работу с текущим binding,
            // а prewarm worker выполнит refresh после завершения startup.
            if self.services.mcp_runtime.is_current_startup_pending() {
