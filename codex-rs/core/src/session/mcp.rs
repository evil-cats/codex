//! Собирает, публикует и обновляет MCP runtime одной сессии Codex.
...
impl Session {
    ...
    /// Публикует изменённое MCP-состояние, не ожидая startup текущего snapshot.
    ///
    /// Семафор сериализует сами refresh, а `McpConnectionSet` решает, можно ли
    /// переиспользовать ещё запускающееся соединение.
