...
impl Session {
    ...
    /// Восстанавливает active history и связанные resume metadata из persisted rollout.
    ///
    /// Media preparation сохраняет envelope metadata, после чего `read_file`
    /// восстанавливает provenance по raw items того же replay.
    #[instrument(
        level = "trace",
        skip_all,
        fields(
    ...
    async fn apply_rollout_reconstruction(
        ...
        let rollout_reconstruction::RolloutReconstruction {
            ...
            .await;
        // Исходный rollout не меняется. Подготовка восстановленной history до её
        // установки обрабатывает legacy media один раз для этого resume/fork и
        // повторит обработку при следующей реконструкции того же rollout.
        // Replay отключает image-resize notices, поэтому преобразование остаётся
        // взаимно-однозначным, а history-only metadata переносится позиционно.
    ...
    ) -> CodexResult<WorldStateDelivery> {
    ...
    /// Строит канонический initial context с persistent и contextual developer sections.
    ...
    /// Возвращает логический ключ окна для fork-механизмов и UUID для метаданных запроса.
    pub(crate) async fn current_window(&self) -> (String, Uuid) {
        ...
        (self.context_window_id(window_number), context_window_id)
    }
