//! Выполняет streaming remote V2 compaction и устанавливает metadata-aware replacement history.
...
/// Устанавливает Remote V2 replacement без retained extension-owned prompt state.
async fn run_remote_compact_task_inner_impl(
    ...
    let compacted_history = history_item_groups(compacted_history)
        .filter(|group| !sess.is_extension_model_context_message(&group.source.item))
