...
impl ContextManager {
    ...
    /// Продвигает snapshot и возвращает history/next-sampling diff вместе с rollout patch.
    pub(crate) fn update_world_state(
        ...
        let fragments = world_state
            .render_history_diff_for_sampling(self.world_state_baseline.as_ref(), self.raw_items());
    ...
    /// Записывает API items, применяя payload policy без потери history-only metadata.
        ...
        T: Deref<Target = ResponseItem>,
    {
        for (item, metadata) in items {
            ...
            let output_history_policy = ToolOutputHistoryPolicy::for_item(item, self.raw_items());
            let processed = ResponseItemEnvelope {
                item: Self::process_item(item, policy, output_history_policy),
    ...
    /// Применяет выбранную политику усечения, сохраняя идентичность исходного item.
    ///
    /// Для `FunctionCallOutput` меняется только `output`; `call_id`, `name`,
    /// `namespace` и внутренняя metadata переносятся без изменений.
    fn process_item(
        item: &ResponseItem,
        policy: TruncationPolicy,
