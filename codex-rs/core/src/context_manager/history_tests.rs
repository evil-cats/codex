...
/// Проверяет, что typed-пара `read_file` сохраняет уже ограниченный output целиком.
#[test]
...
/// Проверяет, что текстовый префикс `ReadFile` не отключает лимит для другого tool.
#[test]
