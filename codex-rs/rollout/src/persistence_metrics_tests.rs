...
#[test]
fn filtered_core_tool_activity_completion_includes_its_nested_item_type() {
    let item = RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
        thread_id: ThreadId::default(),
        turn_id: "turn".to_string(),
        item: TurnItem::CoreToolActivity(CoreToolActivityItem {
            id: "item".to_string(),
    ...
    assert_eq!(
        ...
        "event.item_completed.core_tool_activity"
    );
    assert_eq!(
