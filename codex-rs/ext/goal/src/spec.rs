...
pub fn create_update_goal_tool() -> ToolSpec {
    let properties = BTreeMap::from([(
        ...
        JsonSchema::string_enum(
            vec![json!("complete"), json!("blocked"), json!("cancel")],
            Some(
                "Required. Set to `complete` only when the objective is achieved and no required work remains. Set to `blocked` only after the same blocking condition has recurred for at least three consecutive goal turns and the agent is at an impasse. Set to `cancel` to immediately remove the goal without applying the blocked audit. After a previously blocked goal is resumed, the resumed run starts a fresh blocked audit."
                    .to_string(),
            ),
        ),
    )]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: r#"Update or cancel the existing goal.
Use this tool only to mark the goal achieved, genuinely blocked, or intentionally abandoned.
...
Set status to `cancel` when the goal must be abandoned rather than completed or blocked. Cancellation is immediate, removes the goal, and does not require the blocked audit.
