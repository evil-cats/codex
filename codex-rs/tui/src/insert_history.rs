    ...
    B: Backend<Error = io::Error> + Write,
{
    ...
    insert_history_items_with_mode_and_wrap_policy(
        terminal,
        items,
...
where
    B: Backend<Error = io::Error> + Write,
{
    let items = lines.iter().cloned().map(HistoryInsertItem::Line).collect();
    insert_history_items_with_mode_and_wrap_policy(terminal, items, mode, wrap_policy, screen_size)
}

pub(crate) fn insert_history_items_with_mode_and_wrap_policy<B>(
    terminal: &mut crate::custom_terminal::Terminal<B>,
    items: Vec<HistoryInsertItem>,
    mode: InsertHistoryMode,
    wrap_policy: HistoryLineWrapPolicy,
