...
// Фоновая очистка сразу закрывает локальные источники событий и затем выгружает
// больше не нужный side runtime, не позволяя поздним событиям вернуть его в UI.
#[tokio::test]
async fn background_side_cleanup_unloads_runtime_and_ignores_late_events() -> Result<()> {
    ...
    time::timeout(Duration::from_secs(/*secs*/ 2), async {
        loop {
            if !app_server_loaded_thread_ids(&mut app_server)
                .await?
                .contains(&side_thread_id.to_string())
            {
    ...
    .await
    .expect("background side cleanup should unload its runtime")?;
...
-> Result<()> {
    ...
    let rendered = app.render_transcript_lines_for_reflow(/*width*/ 80);
    assert_eq!(rendered.items.len(), 7);
    assert_snapshot!(
...
// Проверяет, что новый `ChatWidget` наследует кэш уже выведенного terminal title
// при замене через общий вспомогательный метод жизненного цикла.
#[tokio::test]
async fn replace_chat_widget_preserves_terminal_title_cache() {
    let (mut app, _app_event_rx, _op_rx) = make_test_app_with_channels().await;
    app.chat_widget.last_terminal_title = Some("hermione".to_string());
    ...
    let mut replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        config: app.config.clone(),
        frame_requester: crate::tui::FrameRequester::test_dummy(),
        app_event_tx: app.app_event_tx.clone(),
        workspace_command_runner: None,
    ...
    replacement.last_terminal_title = None;
    assert_eq!(replacement.last_terminal_title, None);
    app.replace_chat_widget(replacement);
    assert_eq!(
        app.chat_widget.last_terminal_title,
        Some("hermione".to_string())
...
async fn clear_ui_unloads_previous_thread_runtime() -> Result<()> {
    Box::pin(async {
        ...
        let control =
            Box::pin(app.handle_event(&mut tui, &mut app_server, AppEvent::ClearUi { name: None }))
                .await?;
