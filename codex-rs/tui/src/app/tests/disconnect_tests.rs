...
/// Проверяет, что фильтр состояния `offline` сохраняет резервный текст и
/// типизированный маркер доверенного локального изображения.
#[tokio::test]
async fn offline_dispatch_preserves_local_image_history_preview() -> Result<()> {
    ...
    let image_path = image_dir.path().join("offline-preview.png");
    ...
    let control = app
        .handle_event(
            ...
            AppEvent::InsertLocalImage {
                ...
                caption: Some("offline preview".to_string()),
    ...
    let cell = app
        .transcript_cells
        .last()
        .expect("offline local image should be retained in transcript history");
    let display_items = cell.display_items_for_mode(
        /*width*/ 80,
        crate::history_cell::HistoryRenderMode::Rich,
    );
    assert_matches!(
        display_items.as_slice(),
        [
            HistoryCellDisplayItem::Line(_),
