...
async fn resize_reflow_preserves_configured_scrollback_beyond_the_visible_viewport() {
    ...
    assert_eq!(rendered.items.len(), 32);
    assert!(rendered.items.len() > usize::from(visible_history_rows));
    assert_eq!(
        rendered.items.last().map(rendered_line_text),
...
async fn initial_resume_replay_retains_scrollback_beyond_the_visible_viewport() -> Result<()> {
    ...
    let retained_items = &app
        .initial_history_replay_buffer
        .as_ref()
        .expect("initial replay buffer should remain active")
        .retained_items;
    assert_eq!(retained_items.len(), 32);
    assert!(retained_items.len() > usize::from(visible_history_rows));
    assert!(
...
async fn resize_reflow_preserves_configured_scrollback_when_the_terminal_height_changes() {
    ...
    assert_eq!(smaller.items.len(), 48);
    assert_eq!(larger.items.len(), 48);
    assert_eq!(
        smaller
            .items
            .iter()
    ...
    assert_eq!(
        larger.items.last().map(rendered_line_text),
        Some("cell 63".to_string())
    );
}
...
async fn resize_reflow_preserves_explicitly_unlimited_history() {
    ...
    assert_eq!(
        rendered.items.first().map(rendered_line_text),
...
async fn capped_resize_reflow_prepends_transcript_notice_without_changing_transcript() {
    ...
    assert_eq!(rendered.items.len(), 8);
