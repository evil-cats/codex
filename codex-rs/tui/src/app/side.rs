...
impl App {
    ...
    pub(super) async fn discard_side_thread_in_background(
        ...
        let unload_request_id = app_server.next_request_id();
        ...
        tokio::spawn(async move {
            ...
            if let Err(error) = request_handle
                .request_typed::<ThreadUnloadResponse>(ClientRequest::ThreadUnload {
                ...
                .await
            {
                tracing::warn!(%error, "failed to unload side conversation");
            }
        });
    }
