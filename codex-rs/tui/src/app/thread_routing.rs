...
impl App {
    /// Возвращает все runtime, которыми владеет текущий lifecycle TUI threads.
    ///
    /// Side runtimes включаются явно, чтобы выгрузка предыдущего набора runtime при переходе на
    /// новый primary thread не зависела от сохранности их event channels.
    pub(super) fn tracked_runtime_thread_ids(&self) -> Vec<ThreadId> {
        ...
        for thread_id in self.side_threads.keys().copied().chain(
