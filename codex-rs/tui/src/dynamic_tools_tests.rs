...
fn core_tool_activity_and_other_metadata_are_retained_without_outputs() -> color_eyre::Result<()> {
    let turn: Turn = serde_json::from_value(json!({
        ...
        "items": [
            ...
            {"type": "coreToolActivity", "id": "core-tool-1", "toolName": "read_file",
                "kind": "file", "detail": "main.rs", "arguments": {"path": "src/main.rs"},
    ...
    assert_eq!(
        ...
        json!([
            ...
            {"type": "coreToolActivity", "id": "core-tool-1", "toolName": "read_file",
                "kind": "file", "detail": "main.rs", "arguments": {"path": "src/main.rs"},
                "status": "completed", "error": null, "durationMs": null},
            {"type": "userMessage", "id": "user-1", "content": [
                {"type": "text", "text": delegated_prompt("source-1", "Check <main> & status"),
    ...
    assert_eq!(no_outputs["items"][6]["text"], "Working");
    assert_eq!(
        no_outputs["items"][1]["output"],
        json!({"text": "", "truncated": true, "originalChars": 14})
    );
    assert_eq!(
        no_outputs["items"][9]["result"],
