//! Управляет жизненным циклом терминала, встроенной областью вывода и вставкой TUI history в scrollback.
...
impl Tui {
    ...
    /// Записывает накопленные history items над viewport и очищает буфер.
    fn flush_pending_history_lines(
        ...
        for batch in pending_history_lines.iter() {
            ...
            crate::insert_history::insert_history_items_with_mode_and_wrap_policy(
                terminal,
                batch.items.clone(),
                mode,
                batch.wrap_policy,
