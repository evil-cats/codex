    ...
    fn display_hyperlink_lines(&self, width: u16) -> Vec<HyperlinkLine> {
        self.rich_display_items(width)
            ...
            .filter_map(|item| match item {
                HistoryCellDisplayItem::Line(line) => Some(line),
...
impl UserHistoryCell {
    /// Собирает строки расширенного режима с метаданными ссылок и доверенными `LocalImage`.
    fn rich_display_items(&self, width: u16) -> Vec<HistoryCellDisplayItem> {
        ...
        if !self.remote_image_urls.is_empty() {
            let wrapped_remote_images = plain_hyperlink_lines(adaptive_wrap_lines(
                self.remote_image_urls
                    .iter()
                    .enumerate()
                    .map(|(idx, _url)| image_display_line(element_style, idx.saturating_add(1))),
                RtOptions::new(usize::from(wrap_width))
                    .wrap_algorithm(textwrap::WrapAlgorithm::FirstFit),
            ));
            items.extend(
                prefix_hyperlink_lines(wrapped_remote_images, "  ".into(), "  ".into())
                    .into_iter()
        ...
        for (idx, path) in self.local_image_paths.iter().enumerate() {
            let index = self
                ...
                .saturating_add(idx)
                .saturating_add(1);
            let wrapped_label = plain_hyperlink_lines(adaptive_wrap_lines(
                [image_display_line(element_style, index)],
                RtOptions::new(usize::from(wrap_width))
                    .wrap_algorithm(textwrap::WrapAlgorithm::FirstFit),
            ));
            items.extend(
                prefix_hyperlink_lines(wrapped_label, "  ".into(), "  ".into())
                    .into_iter()
                    .map(HistoryCellDisplayItem::Line),
            );
        ...
        if attachment_count > 0 && wrapped_message.is_some() {
            items.push(HistoryCellDisplayItem::from(Line::from("").style(style)));
        }

        if let Some(wrapped_message) = wrapped_message {
            items.extend(
                prefix_hyperlink_lines(wrapped_message, "› ".bold().dim(), "  ".into())
                    .into_iter()
                    .map(HistoryCellDisplayItem::Line),
            );
        }
