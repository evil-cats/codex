...
#[test]
// Проверяет, что ячейка текущей сессии не усекает подтверждённый многострочный ввод.
fn terminal_interaction_cell_renders_full_input() {
...
fn final_separator_token_usage_reports_running_agents_on_frozen_totals() {
    ...
    let cell =
        FinalMessageSeparator::new(/*elapsed_seconds*/ None, /*runtime_metrics*/ None)
            .with_root_turn_token_usage(RootTurnTokenUsageSnapshot {
                turn: TokenUsageSnapshot::default(),
                agents: agents.clone(),
                total: agents,
                agent_count: 1,
...
/// Проверяет, что доверенный `LocalImage` не уничтожает метаданные ссылок соседнего сообщения.
#[test]
fn user_history_cell_preserves_hyperlinks_alongside_local_image_items() {
    let url = "https://example.test/forwarded/threads/10930?queue=customer_support_unprocessed";
    let image_path = PathBuf::from("/tmp/example.png");
    let cell = UserHistoryCell {
    ...
    let display_items = cell.display_items_for_mode(/*width*/ 36, HistoryRenderMode::Rich);
    ...
    let linked_lines = display_items
        .iter()
        .filter_map(|item| match item {
            HistoryCellDisplayItem::Line(line) if !line.hyperlinks.is_empty() => Some(line),
            HistoryCellDisplayItem::Line(_) | HistoryCellDisplayItem::LocalImage { .. } => None,
        })
        .collect::<Vec<_>>();
    assert!(!linked_lines.is_empty(), "ожидались метаданные ссылки");
    assert!(linked_lines.iter().all(|line| {
        line.hyperlinks
            .iter()
            .all(|hyperlink| hyperlink.destination == url)
    }));
