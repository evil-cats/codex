...
impl McpRuntime {
    ...
    /// Ждёт отказа переиспользованного pending startup только в актуальном snapshot.
    /// При смене snapshot повторяет проверку; возвращает `false`, если наблюдать
    /// нечего или актуальный startup завершился без `Failed`.
    pub async fn wait_for_current_reused_pending_startup_failure(&self) -> bool {
        loop {
            ...
            let failed = current
                ...
                .await;
            if Arc::ptr_eq(&current, &self.current.load_full()) {
                return failed;
            }
        }
    }
    ...
    /// дополнительные транспорты. Прочие данные snapshot остаются пустыми.
    #[cfg(test)]
