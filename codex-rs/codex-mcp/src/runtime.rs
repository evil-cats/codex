...
impl McpRuntime {
    ...
    /// Показывает, выполняется ли startup хотя бы одного соединения текущего snapshot.
    pub fn is_current_startup_pending(&self) -> bool {
        self.current.load().connections.is_startup_pending()
    }

    /// Ждёт завершения startup snapshot, опубликованного в момент вызова.
    pub async fn wait_for_current_startup(&self) {
        let current = self.current.load_full();
        current.connections.wait_for_startup_settled().await;
    }
