...
/// Итог startup-клиентов, переиспользованных этим snapshot до их готовности.
#[derive(Clone, Copy)]
enum ReusedPendingStartupOutcome {
    Pending,
    SettledWithoutFailure,
    Failed,
}
...
pub(crate) struct McpConnectionSet {
    ...
    reused_pending_startup_outcome: watch::Receiver<ReusedPendingStartupOutcome>,
}

impl McpConnectionSet {
    /// Создаёт набор MCP-соединений и согласует его с предыдущей публикацией.
    ///
    /// Совместимый pending startup переиспользуется без ожидания. Его отказ
    /// наблюдается отдельно, чтобы владелец runtime мог запустить одну замену.
    /// Вызывающий код без thread может не передавать `tx_event`: тогда уведомления
    /// startup пропускаются, а интерактивные elicitation отклоняются.
    pub async fn new(
        ...
        let mut reused_pending_startups = JoinSet::new();
                        ...
                        })
                {
                    ...
                    if let Some(error) = unchanged_auth_failure {
                        ...
                        if !client.is_codex_apps_mcp_server {
                            ...
                            reused_pending_startups.spawn(async move {
                                matches!(
                                    observed_client.client().await,
        ...
        let initial_reused_pending_startup_outcome = if reused_pending_startups.is_empty() {
            ReusedPendingStartupOutcome::SettledWithoutFailure
        } else {
            ReusedPendingStartupOutcome::Pending
        };
        let (reused_pending_startup_outcome_tx, reused_pending_startup_outcome) =
            watch::channel(initial_reused_pending_startup_outcome);
        if !reused_pending_startups.is_empty() {
            tokio::spawn(async move {
                let failed = reused_pending_startups
                    .join_all()
                    .await
                    .into_iter()
                    .any(std::convert::identity);
                let outcome = if failed {
                    ReusedPendingStartupOutcome::Failed
                } else {
                    ReusedPendingStartupOutcome::SettledWithoutFailure
                };
                let _ = reused_pending_startup_outcome_tx.send(outcome);
            });
        }
        let manager = Self {
            servers,
            ...
            reused_pending_startup_outcome,
        };
    ...
    pub fn empty(prefix_mcp_tool_names: bool) -> Self {
        Self {
            ...
            reused_pending_startup_outcome: watch::channel(
                ReusedPendingStartupOutcome::SettledWithoutFailure,
            )
            .1,
        }
    ...
    /// Ждёт переиспользованный pending startup и сообщает, нужна ли замена после отказа.
    pub(crate) async fn wait_for_reused_pending_startup_failure(&self) -> bool {
        let mut outcome = self.reused_pending_startup_outcome.clone();
        loop {
            match *outcome.borrow() {
                ReusedPendingStartupOutcome::Pending => {}
                ReusedPendingStartupOutcome::SettledWithoutFailure => return false,
                ReusedPendingStartupOutcome::Failed => return true,
            }
