...
- `thread/unload` — unload a live in-memory thread runtime without archiving, deleting, or changing its persisted rollout. Loaded threads are shut down through the normal runtime teardown path, return `status: "unloaded"`, and emit `thread/closed`; already-unloaded threads return `status: "notLoaded"`.
...
- `turn/completed` — `{ threadId, turn, tokenUsage }` where `turn.status` is `completed`, `interrupted`, or `failed`; successful turns include their final agent message when available, and failures carry `{ error: { message, codexErrorInfo?, additionalDetails?, misalignment? } }`. For a non-interrupted live root turn, `tokenUsage` is an immutable per-model snapshot with `turn`, `agents`, and `total` maps plus `agentCount` and `runningAgentCount`; child turns, interrupted turns, and histories without the snapshot use `null`.
- `turn/diff/updated` — `{ threadId, turnId, diff }` represents the up-to-date snapshot of the turn-level unified diff, emitted after every FileChange item. `diff` is the latest aggregated unified diff across every file change in the turn. UIs can render this to show the full "what changed" view without stitching individual `fileChange` items.
- `turn/plan/updated` — `{ turnId, explanation?, plan }` whenever the agent shares or changes its plan; each `plan` entry is `{ step, status }` with `status` in `pending`, `inProgress`, or `completed`.
- `rawResponse/completed` — internal-only; when `experimentalRawEvents` is enabled on `thread/start`, `thread/resume`, or `thread/fork`, emits `{ threadId, turnId, responseId, model, usage }` once for each upstream Responses API completion. `model` is the slug reported by the upstream response, falling back to the resolved requested model when the server does not report one. `usage` is the exact upstream usage payload mapped to the app-server token breakdown shape and is `null` when the upstream completion omitted usage. Unlike `thread/tokenUsage/updated`, this notification is not accumulated, estimated, persisted, or replayed.

Token snapshots keep one entry per exact model slug. Each entry is `{ model, usage, incomplete }`; `usage: null` means no counters were available, while `incomplete: true` means at least one completed or interrupted call in that model bucket had unknown counters. `total` combines `turn` and `agents` only within matching model slugs and never adds tokens from different models together.
...
- `terminalInteraction` — `{id, callId, processId, stdin}` for confirmed nonempty input written to a command process, including initial `exec_command.stdin` and later `write_stdin` calls. Each write is a separate chronological item and is restored when persisted history is read or resumed; empty polls and failed writes do not create items.
...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, tokenUsage}` describing Multi-Agent V2 lifecycle activity; `kind` is `started`, `interacted`, `interrupted`, `errored`, or `completed`. Terminal `completed`, `errored`, and `interrupted` items carry the agent's cumulative direct per-model usage within the current root turn; non-terminal items and older history use `tokenUsage: null`. `interrupted` is emitted only after the child turn actually aborts, not merely when an interrupt request is accepted. A terminal child outcome is attributed to the parent turn that spawned or restarted it, so its `item/completed` notification may arrive after that turn's `turn/completed` notification and is included with that turn when history is read.
