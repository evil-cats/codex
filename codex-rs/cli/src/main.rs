...
#[clap(
    ...
    // Исполняемый файл иногда вызывают под платформенным именем вроде
    // `codex-x86_64-unknown-linux-musl`: `bin_name` сохраняет `codex` в справке,
    // а `display_name` — в выводе `--version` вместо имени Cargo-пакета.
    bin_name = "codex",
    display_name = "codex",
