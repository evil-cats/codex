//! Асинхронно обслуживает ввод и вывод Unix PTY через неблокирующий master fd.
...
impl PtyIo {
    ...
    /// Запускает чтение вывода и объединяет обычные записи с подтверждаемой
    /// начальной записью, сохраняя выбранную семантику закрытия stdin.
    pub(crate) fn spawn(
        self,
        ...
        let writer_handle = tokio::spawn(async move {
            ...
            while writer_open || initial_stdin_open {
                tokio::select! {
                    ...
                    request = initial_stdin_rx.recv(), if initial_stdin_open => {
                        ...
                        let result = write_all(&fd, &bytes).await;
            ...
            match stdin_close {
                StdinCloseBehavior::SendEof => {
                    // Сохраняем последовательность newline + текущий VEOF из
                    // portable-pty, включая ожидание очереди терминального ввода.
