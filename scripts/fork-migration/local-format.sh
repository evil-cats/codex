...
#
# Скрипт запускается в текущем checkout после предварительной проверки и до
# тестов/сборки. В режиме `check` он проверяет форматирование через
...
Назначение:
  ...
  нужна, чтобы не путать форматирование с тестами, генераторами и сборкой, а
  также чтобы весь подробный вывод всегда уходил в лог.
