...
# рабочее дерево и не применяет patch; его задача - выполнить тестовый gate в
# уже подготовленном checkout.
#
...
# Для удаленного запуска используй `remote-tests.sh`: он подключится к
# `f-ms-dev` и запустит этот же `local-tests.sh` в удаленном checkout.
#
