...
#
# Скрипт запускается в текущем checkout после базовых проверок, когда нужно
# привести сгенерированные schema-файлы к состоянию текущего кода. Он
...
# индекс или хотя бы отметить через `git add -N`, иначе они не попадут в
# `git diff HEAD` и последующий review может их пропустить. Полный вывод пишется в
...
Важно:
  Новые сгенерированные файлы не попадут в `git diff HEAD` сами по себе. Добавь
  их в индекс через `git add` или `git add -N`, чтобы review и итоговый diff
  учитывали их содержимое.
