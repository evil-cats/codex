...
#
# Скрипт запускается на локальном хосте перед форматированием, генераторами,
# удаленными тестами и сборкой. Его задача - быстро подтвердить, что рабочая
# копия находится в ожидаемой ветке миграции, migration-карта существует и не
# содержит незавершенных строк, а в текущем diff нет очевидных механических
# проблем: whitespace-ошибок, конфликтных маркеров и markdownlint-падений в
# fork/migration документации.
#
