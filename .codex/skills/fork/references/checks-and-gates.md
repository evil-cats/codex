...
Локально и на явно выбранных SSH-хостах основной binary по умолчанию
устанавливается в `/usr/local/bin/codex-hermione`, а соседний
`codex-code-mode-host` — под каноническим именем в `/usr/local/bin`. Для
локальной установки по умолчанию только итоговый `rsync` запускается через
`sudo`. Для удалённой установки по умолчанию команда до freshness-сборки
проверяет возможность выполнить `sudo` без запроса пароля, затем использует
`sudo -n` для создания каталога и запуска принимающей стороны `rsync`. Сборка,
подготовка временных копий и probes остаются непривилегированными.

Явный `--target PATH` переопределяет основной target и сохраняет прямую
публикацию без неявного `sudo` в обоих режимах, а путь Code Mode host выводится
автоматически. Повторяемый `--host HOST` переключает команду на установку пары
только на явно перечисленные SSH-хосты; SSH aliases, authentication и host keys
принадлежат пользовательской SSH-конфигурации. Старые файлы в
`${HOME}/.local/bin` автоматически не удаляются и могут затенять системную
установку через `PATH`.
...
установленные файлы — показывать полный SHA зафиксированного `HEAD`. Затем один
общий путь `rsync --delay-updates` доставляет пару в локальный каталог либо на
каждый выбранный host. Для системного пути по умолчанию локальная команда
запускает этот шаг через `sudo`, а удалённая команда задаёт принимающей стороне
`sudo -n -- rsync`; оба варианта используют `--no-owner` и `--no-group`.
Remote-режим после доставки запускает функциональные probes и проверку ревизии
по SSH. Это остаётся skill-owned установкой, а не ручным копированием файлов.
`objcopy` должен обновлять уже существующую секцию: добавление новой секции или
поиск произвольной строки `dev` в executable не входят в контракт. Ошибка
`strip`, `objcopy` или проверки временной пары останавливает workflow до
`rsync`, а исходные артефакты не изменяются.

`rsync --delay-updates` не превращает два binary в общую файловую транзакцию.
При нескольких SSH-хостах каждый хост завершается отдельно; общей межхостовой
транзакции и автоматического отката уже опубликованных файлов нет. Удалённый
target должен быть совместим с ОС и архитектурой собранных артефактов; проверка
после установки обнаруживает несовместимость уже после публикации.
...
Перед запуском каждой применимой записи режим `cards` копирует окружение
процесса и удаляет из этой копии `NO_COLOR`. Окружение самого CLI и вызывающей
оболочки не меняется. Поэтому ANSI- и snapshot-проверки не зависят от настройки
цвета в пользовательской оболочке, а запуск не требует внешней обёртки
`env -u NO_COLOR`.

Тесты реализации самого skill, включая `ReleaseFastWorkflowTests`, принадлежат
`fork preflight --scope skill`, который запускает `scripts/fork_cli_tests.py`.
Их нельзя добавлять в `fork-tests.v1` Codex-карточки только потому, что skill
собирает или устанавливает описанные ею артефакты.
...
Обновляй карточку, когда запуск выявил новое постоянное current-state сведение
о реализации, архитектурное ограничение, регрессионный сценарий, owner-файл,
риск или условие переноса и fork-контракт при этом сохраняется. Если результат
требует изменить контракт, сначала получи решение пользователя по правилам
`references/fork-card-contract.md`; сам результат запуска не является таким
решением. Дата запуска, версия и hash бинарника, путь установки либо уже
...
4. Зафиксируй результат каждого общего gate только закрытым статусом
   ...
   уточнил постоянное current-state сведение, проверку, риск или условие
   переноса без изменения fork-контракта, обнови соответствующий нормативный
   раздел карточки. Необходимость изменить контракт возвращает workflow к
   пользовательскому согласованию из `references/fork-card-contract.md`.
