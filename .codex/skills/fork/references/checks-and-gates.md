...
Локально основной binary по умолчанию устанавливается в
`/usr/local/bin/codex-hermione`, а соседний `codex-code-mode-host` — под
каноническим именем в `/usr/local/bin`. Только итоговый `rsync` этой системной
публикации запускается через `sudo`; сборка, подготовка временных копий и probes
остаются непривилегированными. Явный `--target PATH` переопределяет основной
локальный target и сохраняет прямую публикацию без неявного `sudo`, а путь Code
Mode host выводится автоматически. Повторяемый `--host HOST` переключает команду
на установку пары только на явно перечисленные SSH-хосты; SSH aliases,
authentication и host keys принадлежат пользовательской SSH-конфигурации.
Remote target по умолчанию равен `.local/bin/codex-hermione` относительно login
home. Для системного локального пути наличие `sudo` проверяется до freshness
сборки, чтобы заведомо невозможная публикация не запускала дорогую сборку. Старые
файлы в `${HOME}/.local/bin` автоматически не удаляются и могут затенять
системную установку через `PATH`.
...
каждый выбранный host; для системного локального пути по умолчанию этот шаг
запускается через `sudo` с параметрами `--no-owner` и `--no-group`. Remote-режим
после доставки запускает функциональные probes и проверку ревизии по SSH. Это
остаётся skill-owned установкой, а не ручным копированием файлов. `objcopy`
должен обновлять уже существующую секцию: добавление новой секции или поиск
произвольной строки `dev` в executable не входят в контракт. Ошибка `strip`,
`objcopy` или проверки временной пары останавливает workflow до `rsync`, а
исходные артефакты не изменяются.
