...
Одной CLI/TUI-версии `0.152.0+hermione` недостаточно, чтобы определить
...
SemVer `0.152.0` без fork-суффикса. Оба исполняемых файла поэтому должны
...
`codex 0.152.0+hermione`, а отдельная вторая строка содержит
`revision <value>`. `codex-code-mode-host` показывает собственный workspace
SemVer `0.152.0` и такую же вторую строку без fork-суффикса. Bazel-сборка
...
CLI/TUI SemVer `0.152.0+hermione`, workspace SemVer `0.152.0` для
