...
показывать отдельную строку `Revision: <value>` в корневом `--help`. Версия
пакета остаётся его SemVer-версией, а ревизия связывает установленный
runtime-комплект с конкретным Git-коммитом.
...
`codex --version` с `0.151.0+hermione`. `codex-code-mode-host` сохраняет
workspace SemVer `0.151.0`; добавлять ему fork-суффикс или отдельный `--version`
не требуется. Bazel-сборка продолжает получать `STABLE_GIT_COMMIT` через
...
`objcopy` меняет runtime help без перелинковки, а установленная пара получает
...
- Если release payload перестанет равняться 40 байтам, `fork install` должен
  ...
  `objcopy`, а результат обновления — через `--help` после него.
