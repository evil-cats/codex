...
`STABLE_GIT_COMMIT` через существующий контракт stamped binary в `defs.bzl` и
может заполнить ту же секцию во время компиляции. Post-link stamping принадлежит
только skill-owned `fork install` для ELF runtime-комплекта Hermione.
