...
Одной CLI/TUI-версии `0.153.2+hermione` недостаточно, чтобы определить
...
SemVer `0.153.2` без fork-суффикса. Оба исполняемых файла поэтому должны
...
`codex 0.153.2+hermione`, а отдельная вторая строка содержит
`revision <value>`. `codex-code-mode-host` показывает собственный workspace
SemVer `0.153.2` и такую же вторую строку без fork-суффикса. Bazel-сборка
...
CLI/TUI SemVer `0.153.2+hermione`, workspace SemVer `0.153.2` для
