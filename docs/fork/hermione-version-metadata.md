...
created: 2026-06-08
updated: 2026-07-21
source_scope: rust-v0.145.0..hermione-0.145.0
---
...
Hermione через Cargo build metadata: `0.145.0+hermione`.

Карточка также фиксирует update-check изменения: сравнение upstream versions
должно игнорировать build metadata после `+`, чтобы `0.145.0+hermione`
сравнивался как `0.145.0`.

| Поле | Значение |
| --- | --- |
| Статус | `active` |
| Основные commits | `33e804a51`, `67067a90f`, перенос через merge до `671afe3ee`, перенос на `0.140.0`, перенос на `0.141.0`, перенос на `0.142.5`, перенос на `0.143.0`, перенос на `0.144.1`, перенос на `0.144.4`, перенос на `0.144.5`, перенос на `0.144.6`, перенос на `0.145.0` |
| Текущая версия ветки | `0.145.0+hermione` |
...
`0.145.0+hermione` не должен ломать сравнение с `rust-v0.146.0` или
`rust-v0.145.0`.
...
| Файл | Роль |
| --- | --- |
| `codex-rs/Cargo.toml` | Держит `workspace.package.version = "0.145.0"` без Hermione metadata |
| `codex-rs/cli/Cargo.toml` | Задаёт explicit `version = "0.145.0+hermione"` для `codex-cli` |
| `codex-rs/tui/Cargo.toml` | Задаёт explicit `version = "0.145.0+hermione"` для `codex-tui` |
| `codex-rs/Cargo.lock` | Фиксирует `codex-cli` и `codex-tui` как `0.145.0+hermione`, остальные package-блоки workspace без `source` как `0.145.0` |
...
3. В обоих manifests должна стоять explicit version текущего upstream release с
   ...
   ```toml
   version = "0.145.0+hermione"
   ```

4. При следующем upstream release нужно менять base part:
   - `0.145.0+hermione` -> `X.Y.Z+hermione`, если base tag `rust-vX.Y.Z`.
5. `workspace.package.version` должен оставаться обычной upstream base version
   без суффикса `+hermione`, сейчас `0.145.0`.
6. `Cargo.lock` должен отражать явно заданные версии `0.145.0+hermione` для
...
Во время переноса на `0.145.0` `Cargo.lock` должен различать два вида
...
- остальные 130 пакетов workspace, наследующие `workspace.package.version`, — с
  обычной версией `0.145.0`.

После нормализации package-блоков без `source` с версией `0.144.6` оставаться не
должно. Внешние пакеты с полем `source`, их `checksum` и списки `dependencies`
эта нормализация не изменяет.
...
- Доказательство version metadata должно подтверждать, что:
  ...
  - `workspace.package.version` остается обычной upstream base version
    `0.145.0`, без суффикса `+hermione`;
  - остальные 130 package-блоков workspace без `source` имеют обычную версию
    `0.145.0`;
  - package-блоки без `source` с версией `0.144.6` отсутствуют;
...
- Build metadata `+hermione` не должен попадать в upstream tag parsing.
  Upstream latest tags имеют вид `rust-v0.145.0`; `extract_version_from_latest_tag`
...
| --- | --- | --- |
| Upstream `0.145.0` для workspace version | перенесено | "Итоговый контракт", "Карта файлов" |
| Explicit `0.145.0+hermione` для CLI | перенесено | "Итоговый контракт", "Пошаговое воспроизведение" |
| Explicit `0.145.0+hermione` для TUI | перенесено | "Итоговый контракт", "Пошаговое воспроизведение" |
