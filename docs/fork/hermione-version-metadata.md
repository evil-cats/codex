...
created: 2026-06-08
updated: 2026-07-18
source_scope: rust-v0.144.6..hermione-0.144.6
---
...
Hermione через Cargo build metadata: `0.144.6+hermione`.

Карточка также фиксирует update-check изменения: сравнение upstream versions
должно игнорировать build metadata после `+`, чтобы `0.144.6+hermione`
сравнивался как `0.144.6`.

| Поле | Значение |
| --- | --- |
| Статус | `active` |
| Основные commits | `33e804a51`, `67067a90f`, перенос через merge до `671afe3ee`, перенос на `0.140.0`, перенос на `0.141.0`, перенос на `0.142.5`, перенос на `0.143.0`, перенос на `0.144.1`, перенос на `0.144.4`, перенос на `0.144.5`, перенос на `0.144.6` |
| Текущая версия ветки | `0.144.6+hermione` |
...
`0.144.6+hermione` не должен ломать сравнение с `rust-v0.145.0` или
`rust-v0.144.6`.
...
| Файл | Роль |
| --- | --- |
| `codex-rs/Cargo.toml` | Держит `workspace.package.version = "0.144.6"` без Hermione metadata |
| `codex-rs/cli/Cargo.toml` | Задаёт explicit `version = "0.144.6+hermione"` для `codex-cli` |
| `codex-rs/tui/Cargo.toml` | Задаёт explicit `version = "0.144.6+hermione"` для `codex-tui` |
| `codex-rs/Cargo.lock` | Фиксирует `codex-cli` и `codex-tui` как `0.144.6+hermione`, остальные package-блоки workspace без `source` как `0.144.6` |
...
3. В обоих manifests должна стоять explicit version текущего upstream release с
   ...
   ```toml
   version = "0.144.6+hermione"
   ```

4. При следующем upstream release нужно менять base part:
   - `0.144.6+hermione` -> `X.Y.Z+hermione`, если base tag `rust-vX.Y.Z`.
5. `workspace.package.version` должен оставаться обычной upstream base version
   без суффикса `+hermione`, сейчас `0.144.6`.
6. `Cargo.lock` должен отражать явно заданные версии `0.144.6+hermione` для
...
Во время переноса на `0.144.6` `Cargo.lock` должен различать два вида
...
- остальные 130 пакетов workspace, наследующие `workspace.package.version`, — с
  обычной версией `0.144.6`.

После нормализации package-блоков без `source` с версией `0.144.5` оставаться не
...
- Доказательство version metadata должно подтверждать, что:
  ...
  - `workspace.package.version` остается обычной upstream base version
    `0.144.6`, без суффикса `+hermione`;
  - остальные 130 package-блоков workspace без `source` имеют обычную версию
    `0.144.6`;
  - package-блоки без `source` с версией `0.144.5` отсутствуют;
...
- Build metadata `+hermione` не должен попадать в upstream tag parsing.
  Upstream latest tags имеют вид `rust-v0.144.6`; `extract_version_from_latest_tag`
...
| --- | --- | --- |
| Upstream `0.144.6` для workspace version | перенесено | "Итоговый контракт", "Карта файлов" |
| Explicit `0.144.6+hermione` для CLI | перенесено | "Итоговый контракт", "Пошаговое воспроизведение" |
| Explicit `0.144.6+hermione` для TUI | перенесено | "Итоговый контракт", "Пошаговое воспроизведение" |
