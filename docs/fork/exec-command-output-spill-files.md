...
переносе доработки. Исполняемая карта конкретных проверок живет в
...
Запуск проверок не задается этой карточкой. Общий порядок запуска и подготовка
удаленного checkout принадлежат `FORK.md`; конкретные внутренние команды
принадлежат соответствующим wrapper-ам.
...
| Нужны точечные тесты для порога, пути, содержимого файла, `SandboxDenied` и веток вне MVP | смысловое покрытие перенесено в карточку; конкретные команды живут только в `scripts/fork-migration/local-tests.sh` |
| Новая активная fork-карточка должна быть добавлена в `local-tests.sh` | перенесено в карточку и покрывается правкой `scripts/fork-migration/local-tests.sh` |
| Запуск проверок выполняется не из карточки, а после подтверждения карточек по общему порядку fork | намеренно не перенесено как инструкция карточки; источник правила - `FORK.md` |
