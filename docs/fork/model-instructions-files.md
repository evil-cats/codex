...
`hermione-0.145.0`. Она подготовлена как отдельное feature-изменение поверх
...
| Граница изменения | Отдельное feature-изменение после migration commit |
...
Реализация находится в отдельном feature diff поверх опубликованного migration
commit `38b2a768b`.
...
- Бинарник собран, но не устанавливался локально или на удалённый host, потому
  что установка не входила в запрос.
...
На момент завершения первоначальной реализации бинарник ещё не устанавливался.
Локальная и удалённая установка выполняются отдельно по явной команде
пользователя и после выполнения фиксируются в исторических результатах.
