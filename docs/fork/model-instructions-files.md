...
created: 2026-07-24
updated: 2026-07-29
source_scope: hermione-0.146.0..HEAD
---
...
Доработка первоначально реализована и проверена в ветке
...
`origin/hermione-0.145.0`. Аудит исходного кода и проверка текущей рабочей копии
подтвердили сохранение действующего контракта после слияния `rust-v0.146.0`;
повторная правка кода не потребовалась.

| Поле | Значение |
| --- | --- |
| Статус | `active` |
| Статус реализации | `implemented` |
| Проверочный статус | Аудит исходного кода завершён; тест карточки ожидает финальных статусов карты миграции |
...
| Release-fast binary `0.145.0` | Собран и проверен исторически |
| Локальная установка `0.145.0` | `/home/slader/.local/bin/codex-hermione`, проверена исторически |
| Установка `0.145.0` на `f-ms-dev` | `/home/slader/.local/bin/codex-hermione`, проверена исторически |
...
Диапазон `rust-v0.145.0..rust-v0.146.0` не добавляет и не удаляет
`model_instructions_files`: ключ остаётся fork-specific. Существующий upstream
ключ `model_instructions_file` сохранён в `ConfigToml`, `ConfigProfile`,
формировании `Config.base_instructions` и очистке startup-only входов
config-lock. Поэтому функциональность не является retired или заменённой новым
upstream-механизмом.

Upstream изменил соседнюю архитектуру загрузки конфигурации: управляемые
requirements теперь применяются к итоговому `ConfigToml` внутри
`Config::load_config_with_layer_stack` до вычисления базовых инструкций, а
config-lock повторно накладывает точные управляемые значения после очистки
входов времени запуска. Перенесённая реализация встроена в эту архитектуру:

- конфликт двух ключей и чтение `model_instructions_files` выполняются после
  применения requirements к `cfg`;
- runtime `ConfigOverrides.base_instructions` по-прежнему предотвращает чтение
  файлов;
- `model_instructions_files` очищается в `drop_lockfile_inputs` рядом с
  `model_instructions_file`;
- результирующий `Config.base_instructions` остаётся единственным источником
  верхнеуровневого Responses API `instructions`.

### Результат переноса

Слияние сохранило типы конфигурации, нормализацию путей, runtime-сборку,
config-lock, schema и все перечисленные тесты. Принадлежащие карточке пути не
имеют неразрешённых конфликтов. Действующий контракт перенесён без изменения
семантики; рекомендуемый итоговый статус карточки для migration map —
`migrated`.
...
### Результаты миграции `0.146.0`

- Аудит исходного кода `rust-v0.145.0..rust-v0.146.0` и проверка текущего diff
  подтвердили сохранение полного config/runtime/API-контракта.
- Ограниченная проверка `git diff --check` для принадлежащих карточке путей
  завершилась без ошибок.
- Режим списка `fork tests` обнаружил исполняемую карту
  `fork-model-instructions-files`.
- Запуск теста карточки через `fork tests` остановился на gate готовности карты
  миграции: эта карточка ещё имеет статус `inProgress`, а соседние карточки —
  `pending`. Внутренний целевой тест не запускался; прямой обход gate через
  `just` не выполнялся.
- Генераторы, форматирование, быстрая сборка и полный набор тестов на этапе одной
  карточки не запускались; они остаются частью общего проверочного прохода после
  финализации карты миграции.

### Известные падения и пропуски

- Для реализации `0.145.0` известных падений после повторного card-level
  прохода нет.
- Для миграции `0.146.0` тест карточки пока не дошёл до внутреннего целевого
  теста из-за незавершённых статусов карты миграции; это общее блокирующее
  условие порядка миграции, а не установленное падение доработки.
- Полный workspace test suite не запускался: для узкой карточки выполнены
  исторические целевые tests `0.145.0`, а полный проход требует отдельного
...
формат release-бинарника. В ходе реализации `0.145.0` команда `fork build-fast`
успешно собрала и проверила `codex-rs/target/release-fast/codex`.
...
Для миграции `0.146.0` fast build и установка ещё не выполнялись.
